...
- C# 프로그래밍 학습
- 1줄 소개: 직선, 사각형, 원을 그릴 수 있는 그림판 프로그램, 마우스 드래그를 이용해 도형을 그리고 색상과 선 굵기를 조절하며, 이미지를 저장하고 불러와 추가 작업이 가능한 간단한 그림판 프로그램
- 사용한 플랫폼:
...
- 과제3 코드의 실행 스크린샷과 구현 내용 설명
  ![과제3_1 실행화면](img/과제3_1.png)
  ![과제3_2 실행화면](img/과제3_2.png)
  ![펭구 직접 그림](img/펭구.png)
...
- 과제 내용 (위 그림 참조)
  ...
  - 외부 이미지 불러오기 및 그 위에 그림 그리기 기능 구현
  - 기존 기능과의 통합 및 확장

- 구현한 내용(위 그림 참조)

  - OpenFileDialog를 이용하여 이미지 파일을 선택하도록 구현했다.
    사용한 코드: `Image.FromFile(ofd.FileName)`
  - 불러온 이미지를 Bitmap으로 변환하여 캔버스를 교체했다.
    사용한 코드: `canvasBitmap = new Bitmap(img);`
  - Graphics 객체를 재생성하여 이미지 위에 계속 그릴 수 있도록 처리했다.
    사용한 코드: `canvasGraphics = Graphics.FromImage(canvasBitmap);`
  - 불러온 이미지 위에 추가로 도형을 그린 후 다시 저장할 수 있도록 전체 기능을 통합했다.
  - 기존 도형 선택, 색상, 선 굵기 기능이 그대로 적용되도록 구조를 유지했다.